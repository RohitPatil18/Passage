...
class BaseNotification:
    ...
    def relay_notification(self, notification, medium):
        ...
        if settings.NOTIFICATION_ASYNC:
            func.delay(
                notification, self.event[medium], self.recepients, self.context)
        else:
            func(
                notification, self.event[medium], self.recepients, self.context)
...
class BaseMail:
    ...
    def get_subject(self):
        assert self.subject, (
            ...
            "or override the `get_subject()` method."
    ...
    def get_email_body(self, context):
        assert self.template_path, (
            ...
            "or override the `get_email_body()` method."
            % self.__class__.__name__
        )
    ...
    def send_email(self):
        ...
        log = EmailLog.objects.create(
            **dataset, notification=self.notification)
